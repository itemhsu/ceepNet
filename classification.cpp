...
std::vector<float> Classifier::Predict(const cv::Mat& img) {
  ...
  std::cout << "input_geometry_.height,"<< input_geometry_.height << "\n";
  std::cout << "input_geometry_.width,"<< input_geometry_.width << "\n";
...
void Classifier::Preprocess(const cv::Mat& img,
  ...
  */
  ...
  sG=1.0/sG/255.0;
  sB=1.0/sB/255.0;
  std::cout << mR <<","<<  mG <<","<< mB <<","<< sR <<","<< sG <<","<< sB <<","<< "185\n";
  mean_ = (cv::Mat_<float>(3, 1) << 0.485, 0.456, 0.406);
  std::cout << "187\n";
  std_ = (cv::Mat_<float>(3, 1) << 0.229, 0.224, 0.225);
  std::cout << "189\n";
...
int main(int argc, char** argv) {
  ...
  string model_file   = "2deepNet.prototxt";
  string trained_file = "2deepNet.caffemodel";
  ...
  std::vector<float> features = classifier.Classify(img);
  for (int i=0; i<256 ;i++){
    std::cout << i<< " features : " << features[i] <<"\n";
  }
